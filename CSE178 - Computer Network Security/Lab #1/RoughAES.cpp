#include <iostream>
#include <bitset>//May use to modify bits
//#include <string>

using namespace std;//Cuz there will be no future updates

void KeyExpansion(string key, string ExpandedKey[176], unsigned char Sbox[256]){
    unsigned char Rcon[11] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36};

    unsigned char temp1;//Word with 4 bytes
    unsigned char temp2;
    unsigned char temp3;
    unsigned char temp4;

    for(int i = 0; i < 16; i+0){
        for(int j = 0; j < 4; j++){
            ExpandedKey[i] = key[4*j];
            //cout << "ExpandedKey[ " << i << "]: " << ExpandedKey[i] << endl;
            i++;
            ExpandedKey[i] = key[4*j+1];
            //cout << "ExpandedKey[ " << i << "]: " << ExpandedKey[i] << endl;
            i++;
            ExpandedKey[i] = key[4*j+2];
            //cout << "ExpandedKey[ " << i << "]: " << ExpandedKey[i] << endl;
            i++;
            ExpandedKey[i] = key[4*j+3];
            //cout << "ExpandedKey[ " << i << "]: " << ExpandedKey[i] << endl;
            i++;

        }
    }
    //ExpandedKey[0]-to-[16] Filled = Word[0]-to-[4] Filled

    for(int i = 16; i < 176; i++){

        temp1= ExpandedKey[i-4];
        temp2= ExpandedKey[i-4+1];
        temp3= ExpandedKey[i-4+2];
        temp4= ExpandedKey[i-4+3];

        int Rcount = 1;//Rcon Count from 1 to 11 because of i/4 in Rcon[i/4]

        if((i % 4) == 0){

        //RotWord(temp)
            unsigned char ByteShift = temp1;

            temp1 = temp2;
            temp2 = temp3;
            temp3 = temp4;
            temp4 = ByteShift;

        //SubWord(RotWord(temp))
            //Substitute Bytes
            string t1 = temp[0];
            string t2 = temp[1];
            string t3 = temp[2];
            string t4 = temp[3];

            cout <<"t1(temp[0]): " << t1[0] << endl;

            int s1 = int(t1[0]);
            int s2 = int(t2[1]);
            int s3 = int(t3[2]);
            int s4 = int(t4[3]);

            cout <<"s1((int)t1[0]): " << s1 << endl;

            temp[0] = Sbox[s1];
            temp[1] = Sbox[s2];
            temp[2] = Sbox[s3];
            temp[3] = Sbox[s4];

            cout << "temp[0](Sbox[s1]): " << temp[0] << endl;

        //Rcon
            unsigned char tem1 = temp[0];
            unsigned char tem2 = temp[1];
            unsigned char tem3 = temp[2];
            unsigned char tem4 = temp[3];

            temp[0] = tem1 ^ Rcon[Rcount];
            temp[1] = tem2 ^ Rcon[Rcount];
            temp[2] = tem3 ^ Rcon[Rcount];
            temp[3] = tem4 ^ Rcon[Rcount];

            Rcount++;
        }

    }
}



int main(){
    /* The given Plaintext message has the length of 128 bits/16 bytes/16 letters.
        Meaning that the Key is 128 bits/16 bytes/16 letters and the Expanded key has the length of 176 bytes
        Number of Rounds are 11 total.
    */
    string PlainText;
    string CypherText;
    string key;
    string ExpandedKey[156];//Total of 44 words.

    unsigned char Sbox[256] = {
     };

    //Get PlainText
    cout << "Please enter the 16 lettered/128 bits PlainText: ";
    getline(cin, PlainText);
    if(PlainText.size() > 16){//Input is too long
        cout << "%" << endl;
        cout << "% Error: Input is too long. Restart" << endl;
        cout << "%";
        return 0;
    }
    while(PlainText.size() < 16){
        cout << "Input is short. Adding zeroes at the end..." << endl;
        PlainText = PlainText + "0";
    }
    cout << "PlainText: " << PlainText << endl;

    //Get Key
    cout << "Please enter the 16 lettered/128 bits Key: ";
    getline(cin, key);
    if(key.size() > 16){//Input is too long
        cout << "%" << endl;
        cout << "% Error: Input is too long. Restart" << endl;
        cout << "%";
        return 0;
    }
    while(key.size() < 16){
        cout << "Input is short. Adding zeroes at the end..." << endl;
        key = key + "0";
    }

    cout << "Key: " << key << endl;

    //KeyExpansion
    KeyExpansion(key, ExpandedKey, Sbox);


}
