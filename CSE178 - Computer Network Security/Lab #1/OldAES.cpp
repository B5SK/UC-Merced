#include <iostream>
#include <bitset>//May use to modify bits
//#include <string>

using namespace std;//Cuz there will be no future updates

unsigned char Sbox[256] {
     };

void KeyExpansion(unsigned char key[17], unsigned char ExpandedKey[177]){
    unsigned char Rcon[11] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36};

    unsigned char temp1;//Word with 4 bytes
    unsigned char temp2;
    unsigned char temp3;
    unsigned char temp4;

    //cout << "Key in KeyExpansion: " << key << endl;

    for(int i = 0; i < 16; i+0){
        for(int j = 0; j < 4; j++){
            ExpandedKey[i] = key[4*j];
            cout << "ExpandedKey[ " << i << "]: " << ExpandedKey[i] << endl;
            i++;
            ExpandedKey[i] = key[4*j+1];
            cout << "ExpandedKey[ " << i << "]: " << ExpandedKey[i] << endl;
            i++;
            ExpandedKey[i] = key[4*j+2];
            cout << "ExpandedKey[ " << i << "]: " << ExpandedKey[i] << endl;
            i++;
            ExpandedKey[i] = key[4*j+3];
            cout << "ExpandedKey[ " << i << "]: " << ExpandedKey[i] << endl;
            i++;

        }
    }
    //ExpandedKey[0]-to-[16] Filled = Word[0]-to-[4] Filled


    int Rcount = 0;//Rcon Count from 1 to 11 because of i/4 in Rcon[i/4]

    for(int i = 16; i < 176; i= i + 4){

        temp1= ExpandedKey[i-4];
        temp2= ExpandedKey[i-4+1];
        temp3= ExpandedKey[i-4+2];
        temp4= ExpandedKey[i-4+3];

        if(i == 16 || i == 32 || i == 48 || i == 64 || i == 80 || i == 96 || i == 112 || i == 128 || i == 144 || i == 160 || i== 176){

        //RotWord(temp)
            unsigned char ByteShift = temp1;

            temp1 = temp2;
            temp2 = temp3;
            temp3 = temp4;
            temp4 = ByteShift;

        //SubWord(RotWord(temp))
            //Substitute Bytes

            int s1 = int(temp1);
            int s2 = int(temp2);
            int s3 = int(temp3);
            int s4 = int(temp4);

            temp1 = Sbox[s1];
            temp2 = Sbox[s2];
            temp3 = Sbox[s3];
            temp4 = Sbox[s4];

        //Rcon

            temp1 = temp1 ^ Rcon[Rcount];

            Rcount++;
        }

        ExpandedKey[i] = ExpandedKey[i-4] ^ temp1;
        ExpandedKey[i+1] = ExpandedKey[i-4+1] ^ temp2;
        ExpandedKey[i+2] = ExpandedKey[i-4+2] ^ temp3;
        ExpandedKey[i+3] = ExpandedKey[i-4+3] ^ temp4;

    }
}



int main(){
    /* The given Plaintext message has the length of 128 bits/16 bytes/16 letters.
        Meaning that the Key is 128 bits/16 bytes/16 letters and the Expanded key has the length of 176 bytes
        Number of Rounds are 11 total.
    */
    unsigned char* PT;//Plaintext
    unsigned char* CT;//Cyphertext
    unsigned char* K;//key
    unsigned char EK[177];//ExpandedKey

    string PlainText;
    string CypherText;
    string key;
    string ExpandedKey[176];//Total of 44 words.


    //Get PlainText
    cout << "Please enter the 32 lettered/128 bits PlainText: ";
    getline(cin, PlainText);
    if(PlainText.size() > 32){//Input is too long
        cout << "%" << endl;
        cout << "% Error: Input is too long. Restart" << endl;
        cout << "%";
        return 0;
    }
    while(PlainText.size() < 32){
        cout << "Input is short. Adding zeroes at the end..." << endl;
        PlainText = PlainText + "0";
    }
    //cout << "PlainText: " << PlainText << endl;

    //Get Key
    cout << "Please enter the 16 lettered/128 bits Key: ";
    getline(cin, key);
    if(key.size() > 32){//Input is too long
        cout << "%" << endl;
        cout << "% Error: Input is too long. Restart" << endl;
        cout << "%";
        return 0;
    }
    while(key.size() < 32){
        cout << "Input is short. Adding zeroes at the end..." << endl;
        key = key + "0";
    }

    //cout << "Key: " << key << endl;

    PT = (unsigned char*)PlainText.c_str();
    K = (unsigned char*)key.c_str();


    PT[32] = '\0';
    K[32] = '\0';
    EK[176] = '\0';

    //KeyExpansion
    KeyExpansion(K, EK);


}
